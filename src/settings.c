...
#define DATA_FLASH_START  (FLASH_END - FMC_FLASH_PAGE_SIZE)
...
{
    const uint32_t addr =  DATA_FLASH_START + (SIZE_CONFIGURATION * num);
...
{
    uint32_t startAddr = DATA_FLASH_START;
...
static int findValidCfg(struct Configuration *pCfg)
{
    ...
    if (validNum != -1) {
        startAddr += SIZE_CONFIGURATION * validNum;
